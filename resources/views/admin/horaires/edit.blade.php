...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.horaire.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.horaires.update", [$horaire->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.horaire.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $horaire->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('arrivee') ? 'is-invalid' : '' }}" type="text" name="arrivee" id="arrivee" value="{{ old('arrivee', $horaire->arrivee) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('depart') ? 'is-invalid' : '' }}" type="text" name="depart" id="depart" value="{{ old('depart', $horaire->depart) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users[]" id="users" multiple>
                    @foreach($users as $id => $user)
                        <option value="{{ $user->id }}" {{ (in_array($user->id, old('users', [])) || $horaire->users->contains($user->id)) ? 'selected' : '' }}>{{ $user->matricule }} - {{ $user->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <div class="invalid-feedback">
