...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.retard.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.retards.update", [$retard->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="duree">{{ trans('cruds.retard.fields.duree') }}</label>
                <input class="form-control {{ $errors->has('duree') ? 'is-invalid' : '' }}" type="number" name="duree" id="duree" value="{{ old('duree', $retard->duree) }}" step="1" required>
                @if($errors->has('duree'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($retard->user ? $retard->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('traffic') ? 'is-invalid' : '' }}" name="traffic_id" id="traffic_id" required>
                    @foreach($traffic as $id => $traffic)
                        <option value="{{ $id }}" {{ ($retard->traffic ? $retard->traffic->id : old('traffic_id')) == $id ? 'selected' : '' }}>{{ $traffic }}</option>
                    @endforeach
                </select>
